...
// CRITICAL PATH FIX: We are 3 levels deep from root (src/modules/inmate/profile), so use ../../../
const PHOTOS_BASE_DIR = path.join(__dirname, "../../../../uploads/photos");
...
// NOTE: Multer destination must be absolute path if using a temporary upload directory
...
// --- GENERAL PROFILE READ & WRITE ---
router.get("/detinut/:id/general", async (req, res) => {
  try {
    const sql = `
      SELECT D.ID, D.IDNP, D.SURNAME, D.NAME, D.SEC_NAME, TO_CHAR(D.BIRDTH, 'DD.MM.YYYY') as BIRDTH, D.FOLDERPENDING, P.NAME as PENITENCIAR_NAME
